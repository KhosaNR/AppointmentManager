...
namespace Infrastructure.Repositories
{
    public class ClientRepository: GenericRepository<Domain.Entities.Client>, IClientRepository
    {
        ...
        public async Task<Client> GetByLastNameAndPhoneNo(string lastName, string phoneNo)
        {
            return await _context.Set<Client>().Include(p => p.Slots).Where(x=>x.LastName==lastName&& x.PhoneNo== phoneNo).FirstOrDefaultAsync();
        }
    }
